...
load power223_cog.txt
MNI_coord=power223_cog/10;
clear power223_cog
...
for n=n_pos
    for p = n_pos
        if C(n,p) > 0.2
            ...
            surf(X,Y,Z,'FaceColor',col,'EdgeColor','none','FaceAlpha', .5);
        end
    end
end
...
for n=1:length(V)
    if V(n)>0
        surf(x*a+scale*MNI_coord(n,2)+ori(1), y*a+scale*MNI_coord(n,1)+ori(2),z*a+scale*MNI_coord(n,3)+ori(3),'FaceColor',col,'EdgeColor','none','FaceAlpha',0.4);
    elseif V(n)<0
        surf(x*a+scale*MNI_coord(n,2)+ori(1), y*a+scale*MNI_coord(n,1)+ori(2),z*a+scale*MNI_coord(n,3)+ori(3),'FaceColor',[0.8 0.8 0.8],'EdgeColor','none','FaceAlpha',0.4);
